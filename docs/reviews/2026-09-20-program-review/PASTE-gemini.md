# Cover note for Gemini (paste this whole file as the first message, then attach the three bundle files)
...
Read bundles 1 and 2 in full before writing anything. Read bundle 3 as
needed and say which parts you opened. Each bundle is a verbatim
concatenation of files from the repository; each file starts with a line
`===== FILE: <path> =====` so you can cite by path.

Number your findings G1, G2, ... The brief follows. Answer it as written.
