# Amendment A3 closure text, version 3 (DRAFT for Gate A)

*2026-09-21 (Pacific). Version 3, written by the Gate A tier 1 reviewer
and filed beside version 2, which stays on disk unedited. It responds to
the twenty-nine findings in
`experiments/06-mvm-0a-constructed-self-index/reviews/2026-09-21-a3-closure-claude-worktree.md`,
six of them fatal. Nothing John has ruled is changed: the outcome word is
the registered one and stays, the position on the project's gradient
stays, the successor stays, and the two refused claim phrases stay refused
in the same form. What changes is what the block cites, what it says about
the registered other-agent control, which sense of the word "center" it
means where, and whether the registered instrument-failure reading is in
the text at all.*

*Version 1 (`a3-closure-text-draft-2026-09-20.md`) and version 2
(`a3-closure-text-draft-2026-09-21-v2.md`) are both unedited on disk.
Version 2's preamble records what superseded version 1 and is not repeated
here.*

*This is registered text once it lands in
`experiments/06-mvm-0a-constructed-self-index/amendment-a3.md` as a dated
closure block. It has not landed. Under the review protocol nothing is
appended until both tiers have filed, John has ruled, and the closure rule
is satisfied.*

**Three things must happen before this text can be committed, and none of
them is a wording change.** First, the ruling that reschedules the
successor (`docs/rulings/2026-09-20-december-result-roadmap.md`) must be
committed; it is currently untracked, and the successor paragraph below
leaves a marked gap rather than cite a file that is not in the record.
Second, the compute ledger must carry the programme running total on its
two most recent rows, so that the money sentence cites a figure the ledger
contains. Third, the blind-arm reconciliation must live somewhere a
reviewer is allowed to read.
...
validity-gate bin of the same name: the validity gates were clean and no
...
separate them (registration revision 8, "The central claim is narrowed").
The instrument the registration names as the discriminator for that
question, the mid-episode re-indexing probe, has never run.
...
the family bar by 1.94 episodes in four thousand, and is recorded as a
sub-bar pattern measured twice, not a clearance. The registered matched
...
no money cost, and until it runs, only the narrower sentence above is
...
establish it; it runs before any further work on the other agent's revision
value, and its status is carried as an open item on this closure.
...
(`docs/competing-mechanisms-2026-09-20.md`). It registers through Gate A
with both tiers. **[Registration date and the ruling that sets it: to be
filled when `docs/rulings/2026-09-20-december-result-roadmap.md` is
committed. The last committed ruling on the successor's timing is item 5
of the center-as-degree ruling, which says it registers after the
hibernation condition. Version 2 of this text cited the uncommitted
December-result ruling for a target of 2026-10-11; that citation is left
out here rather than made to a file the record does not contain.]** Three
...
**Open items carried on this closure.** The blind-localization arm, whose
status is unreconciled in the last committed ruling on it and which runs
before any further work on the other agent's revision value. The registered
probe target, the marker word, unread by a sensitive instrument at these
positions. Causal patching, unwritten and unrun. The mid-episode re-indexing
...
**Money.** Amendment A3 closed at about $44 of its $100 hard stop, and the
programme at about $226 of its $400 ceiling (`compute-ledger.md`; the
programme figure follows from the ledger's last stated total of ~$215.7
plus the two rows after it, and the ledger should carry it directly before
