...
> Section 12.1 and the source list in section 16 say the project's structured
> status file (`data/project.toml`) still carries a stale spend figure and is
> being corrected in a separate session. That correction has since landed, and
> **no number here changes**: section 12 was written against the compute ledger,
