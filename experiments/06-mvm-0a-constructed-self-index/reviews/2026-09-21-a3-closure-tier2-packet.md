...
*Repaired 2026-09-21 (Pacific), before either session ran. The first version of
this packet handed the reviewer the closure text and five of the records it
cites, three of those only "if the app's context allows"; the closure text cites
nineteen records and two commits. Part 1 of the brief asks the reviewer whether the
record shown to them contains each claim; for most of the citations the honest
answer would have been "no record was shown to me", and the reviewer would have
spent the pass reporting the packet's gaps instead of the text's. That is the
same shape as the defect that stopped the previous version of the closure text
— the red team ledger's finding `RT-145`, that a registration commit may not
rest on a record its reader cannot open. Every record the closure text cites is
now supplied, in full where the file is small enough to paste and as a marked
excerpt where it is not. The list below says which is which.*

*Two things were also brought up to date. The packet now carries the version of
the closure text as revised on 2026-09-21 by the independent citation check
(the three commits `4b4ec99`, `bf79ed2` and `41d44ea`, which corrected citations
that pointed at the wrong record and added several new ones), and the brief now
carries the wording of the outside-review protocol as amended on 2026-09-21,
which strikes the line telling reviewers that a pass finding nothing fatal is
"unlikely". Nothing else in the brief is changed; it is protocol text.*

## What the reviewer is given, and what that costs to paste

This is a long packet. Whole files were preferred wherever a file was small
enough to paste, because an excerpt is a place where the packet, rather than
the text under review, decides what the reviewer sees. Three files are too long
for that and are supplied as excerpts: the red team ledger (about 123,000
characters), the compute ledger (about 49,000), and the independent review of
the Amendment A4 clause (about 47,000). In each case the excerpt is the part
the closure text's citation actually points at, quoted unedited, with the file
named and a note that the whole file exists in the repository. A fourth,
this review protocol itself, is cited once for its closure rule; that rule is
quoted in full in block 1 and the rest of the protocol is left out, since
nothing in the closure text points at it.

Where completeness and usability pulled against each other, this packet chose
completeness for the registered text and the reviews, and usability for the two
ledgers. The two registered documents — the amendment and the pre-registration
— are supplied whole even though together they are about 16,500 words, because
part 1 of the brief asks the reviewer to quote the registration, and an
excerpted registration is exactly the kind of cut that manufactures findings.
John can overturn either call.

All told the pasted material is roughly 59,000 words, about 368,000 characters:
1,200 words of packet text, 44,600 words across the seventeen whole files, and
13,000 words of excerpt. Twenty-one paste blocks per reviewer, forty-two across
the two sessions.

## What to paste, in order

Blocks 1, 19, 20 and 21 are in this file, below the line. Blocks 2 to 18 are
files in the repository, pasted whole.

| # | What to paste | Complete file or excerpt |
|---|---|---|
| 1 | The cover note, the closure rule, and the brief — the section below the line in this file | packet text |
| 2 | `docs/a3-closure-text-draft-2026-09-21-v4.md` — the text under review | complete |
| 3 | `experiments/06-mvm-0a-constructed-self-index/reviews/2026-09-21-a3-closure-claude-worktree.md` — the tier 1 findings | complete |
| 4 | `experiments/06-mvm-0a-constructed-self-index/amendment-a3.md` — the registered amendment the block will be appended to | complete |
| 5 | `experiments/06-mvm-0a-constructed-self-index/pre-registration.md` — the registration, including the loss conditions | complete |
| 6 | `experiments/06-mvm-0a-constructed-self-index/seeds-endpoint-findings.md` — the three-seed endpoint scores | complete |
| 7 | `experiments/06-mvm-0a-constructed-self-index/ceiling-measurement-findings.md` — the ownership-blind ceiling of 1.0 | complete |
| 8 | `experiments/06-mvm-0a-constructed-self-index/ceiling-defect-2026-09-17.md` — the registered defect in that ceiling | complete |
| 9 | `experiments/06-mvm-0a-constructed-self-index/control-learnability-pilot-findings.md` — the 0.3125 control reading | complete |
| 10 | `experiments/06-mvm-0a-constructed-self-index/fitted-position-sweep-findings.md` — the fitted eleven-position sweep | complete |
| 11 | `experiments/06-mvm-0a-constructed-self-index/fitted-position-sweep-findings-CORRECTION-2026-09-20.md` — the correction note carrying the one-in-eleven reach figure | complete |
| 12 | `experiments/06-mvm-0a-constructed-self-index/standardised-refit-findings.md` — the standardised refit | complete |
| 13 | `experiments/06-mvm-0a-constructed-self-index/other-index-position-sweep-findings.md` — the other-agent control's probe half | complete |
| 14 | `experiments/06-mvm-0a-constructed-self-index/powered-position-sweep-findings.md` — the difference-of-averages read of the marker word | complete |
| 15 | `docs/rulings/2026-09-20-center-as-degree.md` — the ruling that a load-bearing self-index is a centre | complete |
| 16 | `docs/rulings/2026-09-20-december-result-roadmap.md` — the ruling carrying the successor's dates | complete |
| 17 | `docs/rulings/2026-09-21-followup-runs-and-blind-arm.md` — the ruling reconciling the blind arm | complete |
| 18 | `docs/competing-mechanisms-2026-09-20.md` — the successor experiment's design note | complete |
| 19 | Appendix A of this file — the red team ledger: the rows and review sections the closure text cites | **excerpt** |
| 20 | Appendix B of this file — the independent review of the Amendment A4 clause: its seventeenth finding | **excerpt** |
| 21 | Appendix C of this file — the compute ledger: its rules, its reconciliation baseline, and the two rows dated 2026-09-19 and 2026-09-20 | **excerpt** |

Two citations in the closure text cannot be pasted at all, because they are
commit identifiers rather than documents: `acd8305`, the commit that landed the
December-result ruling, and `38c006b`, the commit that put the programme
running total on the compute ledger's two most recent rows. The files those two
commits landed are blocks 16, 17 and 21, so the substance is checkable even
though the commit identifiers are not.

One file the closure text does **not** cite is offered as an optional extra if
the app still has room: the requirements the separation clause imposes
(`experiments/06-mvm-0a-constructed-self-index/separation-clause-requirements.md`,
about 3,300 words). The previous version of this packet listed it, and the
ledger excerpt in block 19 refers to it. Nothing in the closure text depends on it.

## If the app will not take all of it

Paste in the order above and stop when the app refuses. The order is already a
priority order: blocks 1 to 5 are the text under review, the tier 1 findings and
the registered text, and nothing useful can be said without them. Then record
in the filed response, by name, which blocks were omitted, so a later reader can
tell a gap in the packet from a gap in the record — the reviewer is asked to do
this in the cover note as well.
...
## Block 1, part 1 — cover note (paste this first)

You are being given a short document for review and, with it, the records that
document cites. Almost every record it cites is here. Where a file was too long
to paste into a chat window, you have been given the part of it the citation
points at, quoted unedited, marked as an excerpt, and named so you can say what
you were not shown.

**Records you have in full.** The text under review; the inside reviewer's
findings on it; the registered amendment and the registration it belongs to;
and records of the measurements and rulings the text cites — the three-seed
endpoint scores, the ownership-blind ceiling measurement and the registered
defect in it, the control-learnability pilot reading, five localization records
(the fitted sweep, its correction note, the standardised refit, the other-agent
control, and the difference-of-averages read of the marker word), three rulings,
and the successor experiment's design note.

**Records you have only as excerpts**, each marked where it appears:

- The red team ledger (`red_team_ledger.md`), about 123,000 characters. You have
  the opening of four of its review sections, two of those sections in full, and
  three individual rows from the other two — that is, every row and every range
  the text under review cites. The rest of the ledger covers earlier reviews the
  text does not cite.
- The compute ledger (`compute-ledger.md`), about 49,000 characters. You have
  its rules, its reconciliation baseline, and the two rows the text cites, in full.
- The independent review of a later draft clause (`red-team-a4.md`), about
  47,000 characters. You have its front matter, its summary row and the one
  finding the text cites, its seventeenth, labelled F17 there.
- The review protocol this pass runs under (`docs/outside-review-protocol.md`),
  about 20,000 characters. The text under review cites it once, for its closure
  rule, and that rule is quoted in full in part 2 below. The rest of the
  protocol describes the three review gates, the two tiers and the required
  measurement rehearsal, none of which the text under review cites.

**What this means for your review.** When you cannot check a claim, say which of
these two things is true: the record was not shown to you, or the record was
shown to you and does not contain what the sentence says. The second is a
finding about the text. The first is a limitation of this packet, and is worth
reporting as such but is not a defect in the text under review. If you were not
given something you needed, name the file.
