#!/bin/bash
# launch_ctl_pilot.sh — UNREGISTERED control-learnability pilot launcher.
# HUMAN-RUN ONLY [C2]: this script creates a billable pod and starts a
# training run; Claude prepares it, John executes it, and the go is quoted
# verbatim in the ledger row.
#
# WHY THIS IS A SEPARATE FILE, AND NOT A FLAG ADDED TO launch_a3.sh.
...
# trainer, the frozen batteries and the gates. Registered text does not
# change without John's ruling. It also has no hook for an extra training
# argument -- TRAIN_CMD is built inline -- so staging this pilot "through
# launch_a3.sh with the flag" would have meant editing a registered
# instrument, or smuggling arguments through ACT_WEIGHT, which would leave
# a ledger row nobody could reconstruct later.
#
# This is the repo's own precedent, applied again. launch_a3.sh itself was
# split from launch_pilot_a1.sh for the same reason, and said so: "That
# launcher is the registered instrument of the A2 five-seed programme and
# the five existing checkpoints were produced by it; editing it would put
# their provenance in doubt."
#
# DERIVED FROM launch_a3.sh VERBATIM. Every ops fix, every incident
# comment and every safety check below is carried over unchanged. What
# differs, and nothing else:
#   * --ctl-weight and --ctl-frac are passed to train_a3.py (the
#     unregistered control-learnability flag, default-off in the trainer)
#   * it REFUSES to launch with CTL_WEIGHT=0, because a run without the
#     flag is an A3 run and belongs in the registered launcher
#   * OUT defaults to a distinct name so it cannot overwrite the existing
#     A3 pilot's checkpoint or artifacts
#
# If John would rather the flag live in launch_a3.sh itself, that is a
# one-line change to registered text and needs his ruling. It was not made
# here.
#
# The A3 launcher is a SEPARATE FILE from launch_pilot_a1.sh on purpose. That launcher is the
...
DRYRUN="${DRYRUN:-}"
OUT="${OUT:-a3ctl_$(echo "$SCALE" | tr 'A-Z' 'a-z')_seed${SEED}}"
CTL_WEIGHT="${CTL_WEIGHT:-2.0}"                # UNREGISTERED pilot flag
CTL_FRAC="${CTL_FRAC:-0.5}"                    # pre-stated in
                                               # control-learnability-pilot.md
if [ "$(printf '%s' "$CTL_WEIGHT" | tr -d '0.')" = "" ]; then
  echo "CTL_WEIGHT is $CTL_WEIGHT (zero): that is an A3 run, not this pilot."
  echo "Use the registered launcher src/launch_a3.sh instead."
  exit 1
fi
...
echo "  UNREGISTERED control-learnability flag: ctl-weight $CTL_WEIGHT, ctl-frac $CTL_FRAC"
...
--ctl-weight $CTL_WEIGHT --ctl-frac $CTL_FRAC \
